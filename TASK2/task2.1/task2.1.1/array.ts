...
const aumentoD2puntos = alumnos.filter((cumplen) =>
  cumplen.promedio >= 4 ? (cumplen.promedio += 2) : 0
);
...
const datos = (function value() {
  ...
  const estudianteMayoresMenorpromedio = () => {
    let estuPromMenorMayores = alumnos.filter(
      (cumplen) => cumplen.promedio < 6 && cumplen.edad > 19
    );
  ...
  const promedioGerneral = () => {
    let promedio = 0;
    const promedioGenerar = alumnos.filter((cumplen) =>
      cumplen.promedio > 0 ? (promedio += cumplen.promedio) : 0
    );
...
console.log(
  "no existe ningun estudiante que cumpla por eso devuelve un lista vacia"
);
console.log("");
console.log("Alumnos ordenados de promedio acendente ");
console.table(datos.ordenAcendentePromedio());
console.log(
  "Unmentar el promedio de los estudiantes  en 2 que  tengan el promedio mayor o igual a cuatro"
);
console.table(datos.aumentar2());
console.log(
  "promedio genral (tener en cuenta que se aplico un aumento de dos a todos)"
);
